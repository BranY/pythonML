...
def train_model(features, labels):
    ...
    return gbm, y_pred, df_eval_label, df_eval_features, df_train_features, df_train_label
...
if __name__ == "__main__":
    ...
    print('Load data...')
    df_train_data = pd.read_csv('/Users/yangwenjia/Code/MLTest/data/contest/1.2.2/train.1.2.2/part-00000', header=None,
                                encoding='utf-8')
    df_test_data = pd.read_csv('/Users/yangwenjia/Code/MLTest/data/contest/1.2.2/test.1.2.2/part-00000', header=None,
                               encoding='utf-8')
    ...
    for i in range(5):
        ...
        gbm, y_pred, y_label, x_features, df_train_features, df_train_label = train_model(df_features, labels)
        ...
        test_mid = gbm.predict(df_train_features, num_iteration=gbm.best_iteration)

        mid.append(tmp)
        test_mid_labels.append(test)
        mid_train_features.append(x_features)

        mid_x_features.append(df_train_features)
        mid_y_labels.append(df_train_label)
        mid_pred.append(test_mid)
    ...
    for i in range(5):
        ...
        tmp_feature = []
        labels_tmp = np.array(mid_pred).transpose()
        xx_tmp = mid_x_features[i]
        for m in range(len(xx_tmp)):
            x = xx_tmp[m].tolist()
            x.extend(labels_tmp[m])
            tmp_feature.append(x)

        extend_tmp_features = pd.DataFrame(tmp_feature).values

        gbm_mid, y_pred, y_label_mid, x_features_mid = train_model_mid(extend_tmp_features, mid_y_labels[i],
                                                                       extend_mid_features, mid_train_labels[i])

        pred_mid = gbm_mid.predict(x_features_mid, num_iteration=gbm_mid.best_iteration)
        mid_res += metric(pred_mid, y_label_mid)

        res_mid.append(pred_mid)
    ...
    for i in range(5):
        for j in range(10):
            print(test_mid_labels[i][j], res_mid[i][j], res[i][j])
        print("下一个指标")
